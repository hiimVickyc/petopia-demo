...
/**
 * IconButtonInfo
 * - 支援「受控 pressed」或「非受控 defaultPressed」
 * - a11y: aria-pressed / aria-label / type="button"
 * - 可客製顏色與尺寸
 */
export default function IconButtonInfo({
  text = '',
  icon,
  pressed,                 // 受控模式（可選）
      ...
      {text ? (
        <span
          className={`
            ...
            ${sizes.text}
          `}
          style={{ color: isPressed ? activeText : inactiveText }}
        >
          {text}
        </span>
      ) : null}
